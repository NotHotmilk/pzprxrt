...
(function(pidlist, classbase) {
	...
	MouseEvent: {
		inputModes: {
			edit: ["number"],
			play: ["futon", "shade", "unshade", "clear"]
		},
		mouseinput: function() {
			// オーバーライド
		...
		inputFuton: function() {
			...
			if (this.inputData <= 3) {
				...
				if (cell.isnull) {
					...
					if (adj === null || adj.isnull || adj.isNum()) {
						this.inputData = 46;
					}
				}
				if (old !== this.inputData) {
					this.mouseCell.drawaround();
				}
			}
		},
		inputFuton2: function() {
			if (this.mouseCell.isnull || !this.inputData || this.inputData <= 3) {
				return;
			}
			var cell = this.mouseCell;

			this.changeHalf(cell);
			if (this.inputData !== 41 && this.inputData !== 46) {
				cell.setQans(this.inputData);
				cell.setQsub(0);
			} else if (this.inputData === 46) {
				cell.setQans(41);
				cell.setQsub(0);
			} else if (cell.qans === 41) {
				cell.setQans(46);
				cell.setQsub(0);
		...
		inputcell_shugaku: function() {
			var cell = this.getcell();
			if (cell.isnull || cell === this.mouseCell || cell.isNum()) {
				return;
			}
			if (this.inputData === null) {
				switch (this.inputMode) {
					case "shade":
						this.inputData = cell.qans !== 1 ? 1 : 3;
						break;
					case "unshade":
		...
		changeHalf: function(cell) {
			...
			if (adj === null) {
				/* nop */
			} else if (adj.qans >= 42 && adj.qans <= 45) {
				adj.setQans(41);
			} else if (adj.qans >= 47 && adj.qans <= 50) {
				adj.setQans(46);
			}
		},
	...
	BoardExec: {
		...
		adjustBoardData: function(key, d) {
			...
			switch (key) {
				case this.FLIPY:
					trans = { 42: 43, 43: 42, 47: 48, 48: 47 };
					break; // 上下反転
				case this.FLIPX:
					trans = { 44: 45, 45: 44, 49: 50, 50: 49 };
					break; // 左右反転
				case this.TURNR:
					trans = {
						42: 45,
	...
	Graphic: {
		...
		drawFutons: function() {
			var g = this.vinc("cell_futon", "crispEdges", true),
				mv = this.puzzle.mouse,
				tc = null,
				adj = null;

			var inputting =
				!mv.mouseCell.isnull && mv.firstPoint.bx !== null && mv.inputData >= 40;
			if (inputting) {
				// ふとん入力中
				tc = mv.mouseCell;
		...
		drawPillows: function() {
			var g = this.vinc("cell_pillow", "crispEdges", true),
				mv = this.puzzle.mouse,
				tc = null,
				adj = null;
		...
		getBorderColor: function(border) {
			...
			if (!mv.mouseCell.isnull && mv.firstPoint.bx !== null) {
				// ふとん入力中
				var tc = mv.mouseCell,
					adj = mv.currentTargetADJ();
	...
	Encode: {
		...
		encodeShugaku: function() {
			...
			for (var c = 0; c < bd.cell.length; c++) {
				...
				if (count === 0) {
					cm += pstr;
				} else if (pstr || count === 30) {
					cm += (5 + count).toString(36) + pstr;
					count = 0;
				}
			}
			if (count > 0) {
				cm += (5 + count).toString(36);
			}
			this.outbstr += cm;
		}
	},
		...
		encodeData: function() {
			this.encodeCell(function(cell) {
				if (cell.qnum >= 0) {
					...
					return (cell.qans - 31).toString(20) + " ";
				} else if (cell.qsub === 1) {
					return "- ";
				} else {
					return ". ";
	...
	AnsCheck: {
		...
		checkFullSizeFuton: function() {
			this.checkAllCell(function(cell) {
				return cell.qans === 41 || cell.qans === 46;
			}, "futonHalf");
		},
		...
		checkFutonAisle: function() {
			...
			for (var c = 0; c < bd.cell.length; c++) {
				...
				if (
					cell.countDir4Cell(function(cell) {
						return cell.isShade();
					}) > 0 ||
					adj.countDir4Cell(function(cell) {
						return cell.isShade();
					}) > 0
				) {
					continue;
