...
(function() {
	...
	(function(Genre, obj) {
		...
		},
		{
			...
			bannedblock: [1, 0, "バンブロ", "Banned Block"],
			"bannedblock-aux": [0, 0, "禁止配置の編集", "Edit banned pattern"],
			...
			climber: [1, 0, "Mountain Climber", "Mountain Climber"],
			...
			celltinels: [0, 0, "Celltinels", "Celltinels"],
			...
			dungeon: [0, 0, "Dungeon Map", "Dungeon Map"],
			...
			onetwothreefour: [0, 0, "ワンツースリーフォー", "One Two Three Four"],
	...
	pzpr.parser.URLData.prototype = {
		...
		outputURLType: function() {
			...
			if (pzpr.env.node) {
				...
				url = "https://nothotmilk.github.io/pzprxrt/p";
			}
			switch (this.type) {
				case URL_PZPRV3:
					url = url + "?%PID%/";
...
pzpr.classmgr.makeCommon({
	...
	Board: {
		...
		updateSolverAnswerForCells: function(result) {
			...
			var hasNoAnswer =
				typeof result === "string" || result.hasAnswer === false;
							...
							switch (item.kind) {
								case "text":
									...
									if (
										...
										if (letterCode >= 65 && letterCode <= 90) {
											solverValue = letterCode - 64;
										} else if (letterCode >= 97 && letterCode <= 122) {
											solverValue = letterCode - 70;
										}
									}
									if (!isFinite(solverValue)) {
										break;
									}
									cell.qansBySolver =
								...
								case "sudokuCandidateSet": // 数独の候補数字など
									if (this.pid === "sudoku") {
										...
										for (let n = 0; n < item.values.length; ++n) {
											const value = item.values[n];
											if (value >= 1 && value <= candidateCount) {
												cell.qcandBySolver[value - 1] = true;
											}
										}
									}
		...
		clearSolverAnswerForCrosses: function() {
			...
			for (var i = 0; i < this.cross.length; ++i) {
				var cross = this.cross[i];
				if (cross.qansBySolver !== 0) {
					cross.qansBySolver = 0;
					needUpdateField = true;
				}
			}
			return needUpdateField;
		...
		updateSolverAnswerForCrosses: function(result) {
			...
			for (var y = 0; y < this.rows * 2 + 1; ++y) {
				...
				dataByCross.push(row);
			}
			var dataRaw = result.data;
			for (var i = 0; i < dataRaw.length; ++i) {
				var elem = dataRaw[i];
				if (elem.color !== "green" || elem.x % 2 !== 0 || elem.y % 2 !== 0) {
					continue;
				}
				dataByCross[elem.y][elem.x].push(elem.item);
			}
			for (var j = 0; j < this.cross.length; ++j) {
				var cross = this.cross[j];
				var data = dataByCross[cross.by][cross.bx];
				for (var k = 0; k < data.length; ++k) {
					if (data[k] === "backslash") {
						cross.qansBySolver = 1;
					} else if (data[k] === "slash") {
						cross.qansBySolver = 2;
					}
				}
			}
		},
	...
	pzpr.classmgr.makeCommon({
		...
		Graphic: {
			...
			initFont: function() {
				...
				this.fontfamily = '"Noto Sans JP", "Neue Haas Grotesk Text Pro",  ' + this.fontfamily;
	...
	Graphic: {
		...
		drawMBs: function(withcross) {
			...
			for (var i = 0; i < clist.length; i++) {
				...
				if (
					withcross !== false &&
					(cell.qsub === 2 || cell.qsubBySolver === 2)
				) {
					if (!cell.trial) {
